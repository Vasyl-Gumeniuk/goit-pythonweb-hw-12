...
contacts = [
    {
        ...
        "lastname": "Gumeniuk",
        "birthday": "1988-07-14",
        "email": "vasyl.gumeniuk@gmail.com",
...
async def test_fetch_contacts_with_filters(client, monkeypatch, auth_headers):
    ...
    response = client.get(
        "/api/contacts/?firstname=Vasyl&lastname=Gumeniuk", headers=auth_headers
    )
    assert response.status_code == 200
    assert len(response.json()) == len(contacts[0])
    assert response.json()[0]["firstname"] == "Vasyl"
    mock_contacts.assert_called_once_with("Vasyl", "Gumeniuk", "", 0, 100, account_info)
